...
// ---- Hash-based value noise FBM (Metal-safe replacement for FastNoiseLite) ----

// Seeded 2D hash → float [0,1]
float hash_value_2d(float x, float y, uint seed) {
...
float value_noise_2d(float x, float y, uint seed) {
    ...
    float c00 = hash_value_2d(fx, fy, seed);
    float c10 = hash_value_2d(fx + 1.0, fy, seed);
    float c01 = hash_value_2d(fx, fy + 1.0, seed);
    float c11 = hash_value_2d(fx + 1.0, fy + 1.0, seed);
    float m0 = mix(c00, c10, tx);
    float m1 = mix(c01, c11, tx);
    return mix(m0, m1, ty) * 2.0 - 1.0;
}
