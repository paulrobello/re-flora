...
vec3 get_wind(vec3 world_pos, float time)
{
    ...
    vec2 direction_noise = vec2(
        fnlGetNoise2D(direction_state, sample_pos.x + direction_time.x, sample_pos.y + direction_time.y),
        fnlGetNoise2D(direction_state, sample_pos.x + WIND_SECOND_SAMPLE_OFFSET.x + direction_time.x, sample_pos.y + WIND_SECOND_SAMPLE_OFFSET.y + direction_time.y)
    );

    float dir_len = length(direction_noise);
    vec2 direction = dir_len < 0.0001f ? vec2(1.0f, 0.0f) : direction_noise / dir_len;

    float strength_noise = fnlGetNoise2D(
        strength_state,
