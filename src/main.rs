...
/// Render feature flags passed to the tracer to selectively disable passes for profiling.
#[derive(Clone, Debug)]
...
impl From<&AppOptions> for RenderFlags {
    fn from(opts: &AppOptions) -> Self {
        Self {
            enable_shadows: !opts.no_shadows,
            enable_denoiser: !opts.no_denoise,
            enable_god_rays: !opts.no_god_rays,
            enable_lens_flare: !opts.no_lens_flare,
            enable_tracer: !opts.no_tracer,
            enable_flora: !opts.no_flora,
            enable_particles: !opts.no_particles,
        }
    }
}
